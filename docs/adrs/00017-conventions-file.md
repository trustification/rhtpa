# 00017. Conventions File and Performance Anti-Pattern Standards
...
### AP-5: Sequential DB Calls (Missing Parallelization)

**What it is**: Multiple independent database queries awaited sequentially when they could
be executed concurrently using `tokio::join!` or `futures::try_join!`.

**Why it matters**: Sequential independent queries add their latencies together. With
`try_join!`, the wall-clock time is the maximum of the individual queries, not their sum.

**Occurrences found (4)**:

| # | File | Line(s) | Description |
|---|------|---------|-------------|
| 1 | `modules/fundamental/src/vulnerability/model/details/mod.rs` | ~47-58 | `find_related()` + `find()` sequential — independent queries |
| 2 | `modules/fundamental/src/sbom/model/mod.rs` | ~99-102 | `describes_packages()` + `SbomHead::from_entity()` sequential (has a `// TODO` comment) |
| 3 | `modules/fundamental/src/purl/model/details/versioned_purl.rs` | ~100-102 | Two `load_one()` calls sequential |
| 4 | `modules/fundamental/src/purl/model/details/versioned_purl.rs` | ~48-77 | `find_related()` + `purl_status` query sequential |

**Convention options**:

- **Option A — Parallelize independent queries**: When two or more database queries in the
  same scope are independent (neither uses the other's result), they MUST be executed
  concurrently using `futures::try_join!` or `tokio::join!`.

  ```rust
  // Approved: parallel independent queries
  let (advisories, scores) = futures::try_join!(
      vulnerability.find_related(advisory_vulnerability::Entity).all(tx),
      advisory_vulnerability_score::Entity::find()
          .filter(advisory_vulnerability_score::Column::VulnerabilityId.eq(vuln_id))
          .all(tx),
  )?;

  // Avoid: sequential independent queries
  let advisories = vulnerability.find_related(...).all(tx).await?;
  let scores = advisory_vulnerability_score::Entity::find()...all(tx).await?;
  ```

- **Option B — Parallelize on hot paths only**: Parallelization is recommended but not
  required. It MUST be applied on API-facing read paths with measurable latency. Internal
  batch/ingestion code may remain sequential for readability.

- **Option C — Keep as-is**: No convention. Sequential queries are acceptable. Parallelization
  is a micro-optimization to apply when profiling indicates a bottleneck.

---

### AP-6: Missing Batch/Bulk Operations
...
### AP-7: Recursive Graph Traversal Without Depth Limits
...
### AP-8: Extra DB round-trips vs JOINs / single-query loading
...
### AP-9: Separate queries for validation vs follow-on work (Optional Improvement)

**What it is**: One query establishes **404 / existence** and supplies an identifier (e.g.
`sbom_id`); a **second** query implements different logic (packages, related packages) and
does **not** re-load the same SBOM tuple as the first call.

**Why it matters**: This is two round-trips for two **separate concerns** (guard + domain
query), not duplicate fetching of the same row for the same purpose. This is a common and
defensible pattern for separation of concerns — not a true anti-pattern. Listed here as an
optional improvement opportunity only.

**Occurrences noted (1)**:

| # | File | Line(s) | Description |
|---|------|---------|-------------|
| 1 | `modules/fundamental/src/sbom/endpoints/mod.rs` | ~447, 555 | `fetch_sbom(id)` for 404 + `sbom_id`, then `fetch_sbom_packages` / `fetch_related_packages(sbom_id, …)` — second path builds different SQL; room to reduce to one round-trip if desired |

**Convention options**:

- **Option A — Pass fetched entities downstream**: When an entity is already loaded for
  validation, pass it (or minimal joined state) into follow-on service calls if that
  removes an extra round-trip without harming boundaries.

- **Option B — Accept but document**: Two-query patterns (existence check + work query) are
  acceptable for separation of concerns; document when intentional.

- **Option C — Keep as-is (Recommended)**: No convention. Two queries per request is
  acceptable for clarity unless profiling shows a need to merge.

---

### AP-10: Missing Database Indexes
...
|---|----------|-------------|-------------|---------|
| AP-1 | N+1 Queries | 5 | Batch with JOINs/IN | A: Batch all, B: Batch reads only, C: Keep |
| AP-2 | Unbounded Queries | 3 | Require limits | A: All queries, B: API only, C: Keep |
| AP-3 | In-Memory Filtering | 2 | Push to SQL | A: All filters, B: Large datasets, C: Keep |
| AP-4 | App-Side Counting | 0 | SQL COUNT | A: All counts, B: Paginated only, C: Keep |
| AP-5 | Sequential DB Calls | 4 | Parallelize | A: All independent, B: Hot paths, C: Keep |
| AP-6 | Missing Batch Ops | 0 | Bulk operations | A: All writes, B: Large collections, C: Keep |
| AP-7 | Recursive Traversal | 2 | Depth limits | A: Iterative + limits, B: Limits only, C: Keep |
| AP-8 | Extra round-trips vs JOINs | 1 | Fewer round-trips / JOINs | A: Prefer JOINs, B: Hot paths, C: Keep |
| AP-9 | Validation + follow-on query | 1 (optional) | Keep as-is* | A: Pass downstream, B: Accept, C: Keep |
| AP-10 | Missing Indexes | 2 tables | Index filtered cols | A: All filtered, B: High-traffic, C: Keep |

\* AP-9 recommends Option C (Keep as-is), unlike all other categories which recommend Option A.
...
| # | Category | Occurrences | Recommended | Options |
|---|----------|-------------|-------------|---------|
| CA-1 | Swallowed Errors | 9 | Log all discards | A: Log all, B: High-severity only, C: Keep |
| CA-2 | Stringly-Typed APIs | 5 | Use enums | A: Enums for all, B: Domain values only, C: Keep |
| CA-3 | Code Duplication | 4 | Extract generics | A: Extract all, B: 3+ occurrences, C: Keep |
| CA-4 | Tight Coupling | 3 | Shared types in common | A: No cross-module, B: Read-only OK, C: Keep |
| CA-5 | Oversized Functions | 5 | Max 100 lines | A: Hard limit, B: Complexity-based, C: Keep |
| CA-6 | Inconsistent Tracing | 8 gaps | Instrument all | A: All public, B: Services only, C: Keep |
| CA-7 | Missing Documentation | 7 files | Document all | A: All public, B: Complex only, C: Keep |
| CA-8 | Mixed Logging | 6 files | Use tracing only | A: tracing only, B: Per-module, C: Keep |
| CA-9 | Magic Numbers | 4 | Named constants | A: All values, B: Domain only, C: Keep |
| CA-10 | Raw SQL Parameterization | 1 | Prohibit concatenation | A: Prohibit, B: Review gate, C: Keep |

> **Next step**: Maintainers review and select an option (A, B, or C) for each category
> during PR review. Selected conventions will be added to
> [`CONVENTIONS.md`](../../CONVENTIONS.md) in a follow-up commit.
