...
[[_keycloak_clients]]
===== Keycloak Clients
 * Create keycloak client json files `client-frontend.json`, `client-cli.json`, `client-testing-manager.json`, and `client-testing-user.json` with the below content
...
**Configuration:**
+
The script accepts configuration via command line arguments. Run `./script.sh --help` to see all available options.
+
**Required Parameters:**
+
...
. _--init-data:_ Directory containing keycloak <<_keycloak_clients,clients>> json files
+
**Optional Parameters:**
+
