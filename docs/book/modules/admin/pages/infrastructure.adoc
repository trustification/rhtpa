...
oc create secret tls keycloak-letsencrypt-tls \
  --cert=${HOME}/.acme.sh/${KEYCLOAK_HOSTNAME}/fullchain.cer \
  --key=${HOME}/.acme.sh/${KEYCLOAK_HOSTNAME}/${KEYCLOAK_HOSTNAME}.key \
...
# Optional: Set external frontend URL if Keycloak has an internal name
SSO_FRONTEND_URL="" # ---> 5

# Role name
TRUSTD_ROLE_NAME="trustd"

# Base hostname for redirect URIs
BASE_HOSTNAME="<your-app-domain>" # ---> 6

# Admin username
TRUSTED_ADMIN_USERNAME="admin" # ---> 7
# Admin user password
TRUSTED_ADMIN_PASSWORD="admin123456" # ---> 8

# Directory containing client JSON files
INIT_DATA="/path/to/client/json/" # ---> 9

# Client secrets for service accounts
CLI_CLIENT_SECRET="<generate-secure-secret>" # ---> 10
TESTING_MANAGER_SECRET="<generate-secure-secret>" # ---> 11
TESTING_USER_SECRET="<generate-secure-secret>" # ---> 12
...
CLIENT_OPTS=()
CLIENT_OPTS+=(-s "redirectUris=[\"http://localhost:*\",\"https://rhtpa.${BASE_HOSTNAME}\",\"https://rhtpa.${BASE_HOSTNAME}/*\"]")
if [[ -n "$ID" ]]; then
...
. _SSO_FRONTEND_URL:_ (Optional) External URL for Keycloak if different from internal URL. Leave empty if not needed
. _BASE_HOSTNAME:_ Base application domain for redirect URIs (e.g., "tpa.apps.ocp.cluster.net")
...
. _CLI_CLIENT_SECRET:_ Secure secret for CLI service account client (generate with openssl rand -hex 32)
. _TESTING_MANAGER_SECRET:_ Secure secret for testing-manager service account client
. _TESTING_USER_SECRET:_ Secure secret for testing-user service account client
