...
oidc:
  clients:
    ...
    cli:
      clientSecret:
        value: REPLACE_WITH_YOUR_CLIENT_SECRET # notsecret, replace
----
...
* Issue a Let's Encrypt certificate for your Keycloak hostname:
+
[source,bash]
----
# Get the cluster domain
export LE_WILDCARD=$(oc get ingresscontroller default -n openshift-ingress-operator -o jsonpath='{.status.domain}')
export KEYCLOAK_HOSTNAME="sso-${NAMESPACE}.${LE_WILDCARD}"

# Issue certificate using DNS validation (requires AWS Route53 credentials)
${HOME}/acme.sh/acme.sh --issue \
  -d ${KEYCLOAK_HOSTNAME} \
  --dns dns_aws
----

* Create a Kubernetes TLS secret from the Let's Encrypt certificate:
+
[source,bash]
----
oc create secret tls keycloak-letsencrypt-tls \
  --cert=${HOME}/.acme.sh/${KEYCLOAK_HOSTNAME}_ecc/fullchain.cer \
  --key=${HOME}/.acme.sh/${KEYCLOAK_HOSTNAME}_ecc/${KEYCLOAK_HOSTNAME}.key \
...
[source,bash]
----
oc create secret generic keycloak-db-secret --from-literal=username=trustify --from-literal=password=<your-secure-password> -n <Installation Namespace>
----
...
spec:
  ...
  http:
    ...
    httpPort: 8180
    httpsPort: 8543
    tlsSecret: keycloak-letsencrypt-tls
  hostname:
...
[[_keycloak_clients]]
===== Keycloak Clients
 * Download the keycloak client JSON files from the repository:
 ** link:keycloak-setup/client-frontend.json[client-frontend.json]
 ** link:keycloak-setup/client-cli.json[client-cli.json]
 ** link:keycloak-setup/client-testing-manager.json[client-testing-manager.json]
 ** link:keycloak-setup/client-testing-user.json[client-testing-user.json]

[[_keycloak_config_script]]
===== Create Realm, Roles, clients, client scopes and users
* Download the Keycloak configuration script: link:keycloak-setup/keycloak-setup.sh[keycloak-setup.sh]
...
----
* Execute the script with appropriate parameters to create the Realm, roles, clients, client scopes, and users
+
**Configuration:**
+
The script accepts configuration via command line arguments. Run `./keycloak-setup.sh --help` to see all available options.
+
**Required Parameters:**
+
. _--kcadm-path:_ Path to kcadm.sh from <<_keycloak_bin_download, downloaded>> directory
. _--keycloak-url:_ Keycloak URL from <<_keycloak_usage_,operator>>
. _--keycloak-admin:_ Keycloak ADMIN username from <<_keycloak_usage_,operator>>
. _--keycloak-admin-password:_ Keycloak ADMIN password from <<_keycloak_usage_,operator>>
. _--base-hostname:_ Base application domain for redirect URIs (e.g., "tpa.apps.ocp.cluster.net")
. _--init-data:_ Directory containing the downloaded keycloak <<_keycloak_clients,client JSON files>>
+
**Optional Parameters:**
+
. _--namespace:_ Namespace for the application (used in redirect URIs)
. _--sso-frontend-url:_ External URL for Keycloak if different from internal URL
. _--trusted-admin-username:_ RHTPA application admin username. Default: `admin`
. _--trusted-admin-password:_ RHTPA application admin password. Default: `<secure-password>` (auto-generated if not provided)
. _--cli-client-secret:_ Secure secret for CLI service account client. Auto-generated if not provided
. _--testing-manager-secret:_ Secure secret for testing-manager service account client. Auto-generated if not provided
. _--testing-user-secret:_ Secure secret for testing-user service account client. Auto-generated if not provided
+
**Example Usage:**
+
[source, bash]
----
./keycloak-setup.sh \
  --kcadm-path /path/to/kcadm.sh \
  --keycloak-url https://sso-tpa.apps.ocp.cluster.net \
  --keycloak-admin admin \
  --keycloak-admin-password <your-admin-password> \
  --base-hostname apps.ocp.cluster.net \
  --namespace tpa \
  --init-data /path/to/keycloak-setup
----
...
This script assumes Keycloak is using certificates from a publicly trusted Certificate Authority (CA) such as Let's Encrypt. If you are using self-signed certificates or certificates from an internal CA, you will need to add `--truststore` and `--trustpass` parameters to the `kcadm()` function with a Java truststore containing your CA certificates.
====
...
[NOTE]
The frontend client redirect URIs are now configured programmatically in the script using the BASE_HOSTNAME variable. If you need to add additional redirect URIs manually:

 * Navigate to Keycloak admin console
 * Go to the **Clients** section and select the `frontend` client
 * Add additional URLs to the **Valid Redirect URIs** field as needed
...
* **cli**: The client secret is set programmatically in the script using the CLI_CLIENT_SECRET variable
* **testing-manager**: The client secret is set using the TESTING_MANAGER_SECRET variable
* **testing-user**: The client secret is set using the TESTING_USER_SECRET variable
* **frontend**: `empty object as {}`
[NOTE]
The frontend client is a public client and does not require a secret. Service account client secrets are configured via the script variables.
...
[source, bash]
----
helm upgrade --install -n $NAMESPACE trustify openshift-helm-charts/redhat-trusted-profile-analyzer  --values PATH_TO_VALUES_FILE --set-string appDomain=$APP_DOMAIN --set tracing.enabled=true --set metrics.enabled=true --set-string collector.endpoint="http://dev-collector:4317"
----
...
* The traces from the above job should be captured and displayed in the Traces UI.
...
==== Troubleshooting OpenTelemetry and Tempo Setup

* **Verify endpoint bindings in collector config:**
+
Ensure receivers have `endpoint: 0.0.0.0:<port>` configuration
+
[source, bash]
----
oc get opentelemetrycollector dev -n <namespace> -o yaml | grep -A 5 "receivers:"
----
...
[source, bash]
----
oc auth can-i create tempo.grafana.com/dev --as=system:serviceaccount:tpa-otel:otel-collector
----
+
Expected output: `yes`
...
[source, bash]
----
oc get tempostack -n <namespace>
oc describe tempostack simplest -n <namespace>
----

* **Check TempoStack gateway logs:**
+
[source, bash]
----
oc logs -n <namespace> -l app.kubernetes.io/component=gateway
----
...
[source, bash]
----
oc logs -n <namespace> -l app.kubernetes.io/name=dev-collector --tail=100
----
...
[source, bash]
----
oc get secret s3-secret -n <namespace> -o yaml
----

* **Test S3 connectivity:**
+
Use the AWS CLI verification steps from <<_verify_s3,Verify S3 Connection>> section:
+
[source, bash]
----
export AWS_ACCESS_KEY_ID=$(oc get secret s3-secret -n <namespace> -o jsonpath='{.data.access_key_id}' | base64 -d)
export AWS_SECRET_ACCESS_KEY=$(oc get secret s3-secret -n <namespace> -o jsonpath='{.data.access_key_secret}' | base64 -d)
export S3_ENDPOINT=$(oc get secret s3-secret -n <namespace> -o jsonpath='{.data.endpoint}' | base64 -d)
export AWS_CA_BUNDLE=<path to tls.crt>
aws s3 ls --endpoint $S3_ENDPOINT
----

* **Check TempoStack compactor logs:**
+
[source, bash]
----
oc logs -n <namespace> -l app.kubernetes.io/component=compactor
----

===== ServiceMonitor Not Collecting Metrics
...
oc describe servicemonitor otel-collector-app-metrics -n <namespace>
----
...
+
Go to Observe -> Metrics in the OpenShift web console and click on "Metrics targets" to verify the ServiceMonitor targets are being scraped.
...
[source, bash]
----
# OpenTelemetry Operator logs
oc logs -n openshift-opentelemetry-operator deployment/opentelemetry-operator-controller-manager
