...
#[test(tokio::test)]
async fn test_licenses_union_spdx_and_cyclonedx(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    let service = LicenseService::new();

    // RED: Before ingestion
    let result_before = service.licenses(Query::default(), Paginated::default(), &ctx.db).await?;
    let baseline_count = result_before.total;

    // GREEN: Ingest SPDX (uses expanded_license) and CycloneDX (uses license.text directly)
    ctx.ingest_documents(["spdx/OCP-TOOLS-4.11-RHEL-8.json", "zookeeper-3.9.2-cyclonedx.json"]).await?;

    // REFACTOR: Verify UNION includes both sources
    let result_after = service.licenses(Query::default(), Paginated { offset: 0, limit: 1000 }, &ctx.db).await?;

    assert!(result_after.total > baseline_count, "Should have more licenses after ingestion");
    ...
    let has_license_ref = result_after.items.iter().any(|l| l.license.contains("LicenseRef-"));
    assert!(!has_license_ref, "Results should not contain raw LicenseRef-");
...
async fn test_get_all_license_info_coalesce(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    let info = service.get_all_license_info(Id::Uuid(sbom_id), &ctx.db).await?
        .expect("Should have license info");

    // REFACTOR: Verify data uses COALESCE - no LicenseRef-, all expanded
    for mapping in &info {
        assert!(!mapping.license_name.contains("LicenseRef-"),
            "license_name should be expanded: {}", mapping.license_name);
        assert_eq!(mapping.license_id, mapping.license_name,
...
async fn test_junction_table_mapping_integrity(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    // RED: Empty junction
    assert_eq!(sbom_license_expanded::Entity::find().count(&ctx.db).await?, 0);
    ...
    for entry in entries {
        let expanded = expanded_license::Entity::find_by_id(entry.expanded_license_id)
            .one(&ctx.db).await?;
        assert!(expanded.is_some(), "Junction should reference valid expanded_license_id");
    }
...
#[test_context(TrustifyContext)]
#[test(tokio::test)]
async fn test_expanded_license_md5_deduplication(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    // RED: Ingest once
    ctx.ingest_document("spdx/OCP-TOOLS-4.11-RHEL-8.json").await?;
    ...
    assert_eq!(count_first, count_second, "MD5 hash should prevent duplicate expanded_text");
...
async fn test_cyclonedx_uses_raw_license_text(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    ctx.ingest_document("zookeeper-3.9.2-cyclonedx.json").await?;

    // GREEN: Verify expanded_license table is NOT used for CycloneDX
    let expanded_count = expanded_license::Entity::find().count(&ctx.db).await?;
    assert_eq!(expanded_count, 0, "CycloneDX should not populate expanded_license");

    // REFACTOR: But licenses() should still return CycloneDX licenses via license.text
    let result = service.licenses(Query::default(), Paginated { offset: 0, limit: 100 }, &ctx.db).await?;
    ...
    assert!(has_cyclonedx_license, "Should have CycloneDX licenses from license.text");
...
async fn test_coalesce_refactoring_correctness(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    for mapping in &info {
        assert!(!mapping.license_name.is_empty(), "COALESCE should never produce empty string");
        assert!(!mapping.license_id.is_empty(), "COALESCE should never produce empty string");

        // Both columns use same COALESCE expression - should match
        assert_eq!(mapping.license_name, mapping.license_id,
...
async fn test_license_count_accuracy(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    let before = service.licenses(Query::default(), Paginated { offset: 0, limit: 1000 }, &ctx.db).await?;
    let baseline_count = before.total;

    // GREEN: Ingest documents with licenses
    ctx.ingest_documents(["spdx/OCP-TOOLS-4.11-RHEL-8.json", "zookeeper-3.9.2-cyclonedx.json"]).await?;

    let after = service.licenses(Query::default(), Paginated { offset: 0, limit: 1000 }, &ctx.db).await?;

    // REFACTOR: Verify count increased and matches actual items
    assert!(after.total > baseline_count, "Should have more licenses after ingestion");

    let all_items = service.licenses(Query::default(), Paginated { offset: 0, limit: 10000 }, &ctx.db).await?;
    assert_eq!(all_items.total, all_items.items.len() as u64,
...
async fn test_license_ordering_by_coalesce(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    assert_eq!(licenses, sorted_licenses, "Licenses should be ordered alphabetically via COALESCE");
    assert!(!licenses.is_empty(), "Should have licenses to verify ordering");
...
async fn test_license_filtering_on_coalesce(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    let apache_results = service.licenses(
        q("Apache"),
    ...
    for license in &apache_results.items {
        assert!(license.license.to_lowercase().contains("apache"),
            "Filtered result should contain 'apache': {}", license.license);
    }
...
async fn test_coalesce_null_handling(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    assert!(result.total > 0, "Should get CycloneDX licenses via COALESCE fallback to license.text");

    for license in &result.items {
        assert!(!license.license.is_empty(), "COALESCE should provide non-empty license text");
    }
...
async fn test_license_pagination_with_count(ctx: &TrustifyContext) -> Result<(), anyhow::Error> {
    ...
    let page1 = service.licenses(Query::default(), Paginated { offset: 0, limit: 5 }, &ctx.db).await?;
    ...
    assert!(total >= page1.items.len() as u64, "Total should be >= page 1 items");

    let page2 = service.licenses(Query::default(), Paginated { offset: 5, limit: 5 }, &ctx.db).await?;
    assert_eq!(page2.total, total, "Total count should be same across pages");
    ...
    for license in &page2_licenses {
        assert!(!page1_licenses.contains(license), "Pages should not contain duplicate licenses");
    }
